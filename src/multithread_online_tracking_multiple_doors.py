...
def online_tracker(in_file, model_path, door_vertices):
    ...
    while True:
        ...
        cv2.imshow(f"Tracking_Stream {in_file}", image_drawn)
        key = cv2.waitKey(1)
        if key == ord('q'):
            break
...
def main(input_file_list, model_path):
    ...
    for input_file in input_file_list:
        ...
        tracker_thread_list.append(threading.Thread(target=online_tracker,
                                                    args=(input_file, model_path, door_vertices), daemon=True))
    for thread in tracker_thread_list:
        print('teste')
        thread.start()
    for thread in tracker_thread_list:
        print('join')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-input_file_list', nargs='+', required=True, help="Input file path")
    parser.add_argument('-model', required=True, help="Model path")

    args = parser.parse_args()

    main(args.input_file_list, args.model)
